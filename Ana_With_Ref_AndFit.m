function[pol_fit,timeFull,chanFull,chanRef] =  Ana_With_Ref_AndFit(refData,chanData,Time)
close all
...
for i=1:9
    ...
    [p] = polyfit(timeFull(1,:),15*chanFull(1,:,i),1);
    pol_fit(i)=p(2);
    ylim(I_Axis_limits);
...
figure(114)
plot(timeFull(:,:),15*chanRef(1,:),'linewidth',2,'color',col(5*10-1,:));
figure(115)

plot(timeFull(:,:),15*lowPassedRefData,'linewidth',2,'color',col(5*10-1,:));
[p] = polyfit(timeFull(:,:),15*lowPassedRefData,1);
pol_fit(10)=p(2);
